...
public class QRQuaFactoryImpl {
     ...
     * @return
     */
    public  static String buildQuaForSysRom(Context context, String configDir) {
//        QRQuaConfigFileParser.initQuaItemInfo(configDir);
    ...
    public static String buildQua(String appSnFlg, String appSnVer, String appPublishType, String appVersionNo, 
        ...
        if (appSnFlg == null || "".equals(appSnFlg)) {  // 参数不合法
            QWupLog.trace("qua", "config -- appSnFlg is err!");
            return "";
        }
        if (appSnVer == null || "".equals(appSnVer)) {  // 参数不合法
            QWupLog.trace( "qua", "config -- appSnVer is err!");
            return "";
        }
        if (appPublishType == null || "".equals(appPublishType)) {  // 参数不合法
            QWupLog.trace("qua", "config -- appPublishType is err!");
            return "";
        }
    ...
    public static String getPhoneModel() {
        try {
            if (QWupStringUtil.isEmpty(PHONE_MODEL_BUILD)) {
                
                PHONE_MODEL_BUILD  = getSysProp("getprop ro.qrom.product.device");
                PHONE_MODEL_BUILD = PHONE_MODEL_BUILD == null ? "" :  PHONE_MODEL_BUILD.trim();
            }
        } catch (Throwable e) {
            QWupLog.trace("qua", "getPhoneModel -> err msg: " + e.getMessage());
        }
    ...
    private static String getRVInfo() {
        ...
        try {
            qRomBrand = getSysProp("getprop ro.qrom.build.brand");
            
            // 获取新属性中rom version
            qRomVerNum = QRQuaSysRomConfigParser.getVNFromProp();
            if (qRomVerNum == null || "".equals(qRomVerNum)) {
                // 兼容老版本及新蜂rom，新版本切换到新属性上
                qRomVerNum = getSysProp("getprop ro.qrom.build.version.number");
            }
        } catch (Exception e) {
            QWupLog.w("qua -> rv ", "getRVInfo -> errMsg = " + e.getMessage());
        }
        ...
        QWupLog.trace("qua",  "getRVInfo -> rv = " + rv);
        return rv;        
    }
