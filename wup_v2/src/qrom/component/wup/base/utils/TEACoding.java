...
public class TEACoding {
    ...
    private byte[] encode(byte[] in, int offset, int len) throws IOException {
        if (in == null) {
            return null;
        }
        plain = new byte[8];
    ...
    public byte[] decodeFromHexStr(String code) {
        return decode(StringUtil.hexStringToByte(code));
    }
    ...
    private byte[] encipher(byte[] in) throws IOException {
        if (in == null) {
            return null;
        }
    ...
    private void encrypt8Bytes() throws IOException {
        for (pos = 0; pos < 8; pos++) {
            if (header) {
